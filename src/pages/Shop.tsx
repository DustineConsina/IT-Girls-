...
const Shop: FC = () => {
  ...
  const products = useMemo<Product[]>(() => {
    ...
    const expanded = variantLabels.flatMap((label, variantIndex) =>
      seedProducts.map((product, productIndex) => {
        const multiplier = variantIndex * seedProducts.length;
        const variantId = multiplier + product.id;
        const priceDelta = variantIndex * 12;
        const updatedPrice = product.price + priceDelta;
        const updatedOriginalPrice = product.originalPrice
          ? product.originalPrice + priceDelta + 10
          : undefined;
        ...
        return {
          ...
          isNew: variantIndex === 0 ? product.isNew : variantIndex >= variantLabels.length - 2 || product.isNew,
